#tournament selection (prob=0.7)+ single point crossover(rate 0.7) + mutation (rate 0.8)  
...
def tournament_selection(fitness, num_parents, population):
    p=0.7
    newPopulation=[]
    ...
    while(len(population)!=len(newPopulation)):
      for i in range(len(population)//2):
         alea=rd.random()
         if(alea<p):
            newPopulation.append(sortPopList[i])
         else:
            newPopulation.append(sortPopList[len(sortPopList)-i-1])
...
def optimize(weight, value, population, pop_size, num_generations, threshold):
    ...
    for i in range(num_generations):
        ...
        #print(population)
        population = tournament_selection(fitness, num_parents, population)
        #print("selection:")
        #print(population)
...
def tournament_selection_and_single_point_crossover(numberOfElements, listOfElements , backPackSize):
