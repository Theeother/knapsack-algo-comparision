...
#determinist selection (best 50%)+ uniform crossover(random generation of the mask)  + mutation (rate 0.8)  
...
def uniform_crossover(parents, num_offsprings1,num_offsprings2):
    A=[]
    B=[]
    parents=parents.tolist()
    for i in range(len(parents[num_offsprings1])):
       if (randint(0,1)==0):
           A.append(parents[num_offsprings1][i])
           B.append(parents[num_offsprings2][i])
       else:
           A.append(parents[num_offsprings2][i])
           B.append(parents[num_offsprings1][i])
...
def optimize(weight, value, population, pop_size, num_generations, threshold):
    ...
    for i in range(num_generations):
        ...
        #print("selection:")
        #print(population)
        population = uniform_crossover(population, num_offsprings1,num_offsprings2)
        #print("crossover")
        #print(population)
...
def determinist_selection_and_uniform_crossover(numberOfElements, listOfElements , backPackSize):
